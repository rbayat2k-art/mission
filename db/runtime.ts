...
async function applySchema() {
  ...
  const scoringColumns = [
    { name: "execution_rank", definition: "TINYINT NULL AFTER priority" },
    { name: "execution_rank_version", definition: "INT NOT NULL DEFAULT 0 AFTER execution_rank" },
  ...
  const missionExecutionRankIndex = await database.prepare("SELECT INDEX_NAME AS indexName FROM INFORMATION_SCHEMA.STATISTICS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'missions' AND INDEX_NAME = 'idx_missions_execution_rank'").first();
  if (!missionExecutionRankIndex) await database.prepare("ALTER TABLE missions ADD INDEX idx_missions_execution_rank (execution_rank, created_at)").run();
