...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const existingMission = await db.prepare("SELECT id, source, assigned_to AS assignedTo, status, started_at AS startedAt, destination_name AS destinationName FROM missions WHERE id = ?")
  ...
  const outcome = await db.transaction<StartOutcome>(async (transaction) => {
    ...
    const mission = await transaction.prepare("SELECT id, source, assigned_to AS assignedTo, status, started_at AS startedAt, destination_name AS destinationName FROM missions WHERE id = ? FOR UPDATE")
      .bind(id).first<MissionRow>();
    ...
    if (!["open", "follow_up", "revision"].includes(mission.status)) {
...
export async function DELETE(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const outcome = await db.transaction<CancelOutcome>(async (transaction) => {
    await transaction.prepare("SELECT id FROM users WHERE id = ? FOR UPDATE").bind(auth.user.id).first();
    const mission = await transaction.prepare("SELECT id, assigned_to AS assignedTo, status, started_at AS startedAt FROM missions WHERE id = ? FOR UPDATE")
      .bind(id).first<{ id: string; assignedTo: string; status: string; startedAt: string | null }>();
