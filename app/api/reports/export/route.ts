...
export async function GET(request: Request) {
  ...
  const header = ["نام کارمند", "نام کاربری", "سرپرست", "کارکرد واقعی قابل‌تأیید دقیقه", "روز حضور", "تاخیر دقیقه", "اضافه‌کار پس از ۹ ساعت دقیقه", "کسری نسبت به ۸:۳۰ دقیقه", "زمان اضافه بر مهلت ۳۰ دقیقه بدون GPS", "خوداظهاری در انتظار دقیقه", "تعداد شروع خوداظهاری", "ماموریت تخصیص‌یافته", "تکمیل‌شده", "موفق", "نیازمند پیگیری", "باز", "معوق", "در انتظار تایید", "نرخ تکمیل", "نرخ به‌موقع", "حضور در مقصد دقیقه", "زمان در مسیر دقیقه", "زمان در حرکت دقیقه", "توقف در مسیر دقیقه", "مسافت ماموریت‌ها کیلومتر", "کل مسافت GPS کیلومتر", "جزئیات مسیر ماموریت‌ها", "مقصدها", "وقفه GPS دقیقه", "مدرک", "امتیاز قطعی", "امتیاز در انتظار", "هزینه کل", "هزینه تاییدشده", "هزینه در انتظار"];
  const rows = report.rows.map(row => [row.fullName, row.username, row.supervisorName, row.attendance.activeMinutes, row.attendance.attendanceDays, row.attendance.lateMinutes, row.attendance.overtimeMinutes, row.attendance.shortfallMinutes, row.attendance.unverifiedGpsMinutes, row.attendance.pendingCorrectionMinutes, row.attendance.selfReportedStartCount, row.missions.assignedCount, row.missions.completedCount, row.missions.successfulCount, row.missions.followUpCount, row.missions.openCount, row.missions.overdueCount, row.missions.pendingCount, `${row.missions.completionRate}%`, `${row.missions.onTimeRate}%`, row.movement.onSiteMinutes, row.movement.travelMinutes, row.movement.movingMinutes, row.movement.stoppedMinutes, row.movement.missionDistanceKm, row.movement.distanceKm, row.movement.missionTrips.map(trip => `${trip.title}: ${trip.travelMinutes} دقیقه مسیر، ${trip.movingMinutes} دقیقه حرکت، ${trip.distanceKm} کیلومتر`).join(" | "), row.movement.destinations.join(" | "), row.integrity.gpsGapMinutes, row.quality.attachmentCount, row.quality.confirmedScore, row.quality.pendingScore, row.finance.total, row.finance.approved, row.finance.pending].map(csv).join(","));
