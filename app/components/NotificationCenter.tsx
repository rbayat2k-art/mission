...
type Props={onOpenMissions:()=>void;onOpenFollowUps?:()=>void;onCounts?:(counts:{unread:number;open:number})=>void;compact?:boolean};

export default function NotificationCenter({onOpenMissions,onOpenFollowUps,onCounts,compact=false}:Props){
  ...
  const openItem=async(item:AppNotification)=>{if(!item.readAt)await fetch("/api/notifications",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:item.id})});if(item.entityType==="mission")onOpenMissions();else if(item.entityType==="follow_up_request"&&onOpenFollowUps)onOpenFollowUps();else await load()};
