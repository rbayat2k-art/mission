...
type ApiMission = { id: string; title: string; description: string; source: "manager" | "employee"; status: string; priority: string; assignedTo?: string; destinationName?: string | null; result?: string | null; report?: string | null; expenseAmount?: number; deadline?: string | null; scorePending: number; scoreConfirmed: number; scorePenalty?: number; scoreNote?: string | null; startedAt?: string | null; employeeName?: string; completedAt?: string | null; createdAt?: string; attemptCount?: number };
...
function EmployeeApp() {
  ...
  const finishWork = async () => {
    ...
    try {
      ...
      const result = await sendJsonOrQueue<{mission:{status:string;needsFollowUp:boolean;scorePending:number;scoreConfirmed:number;scorePenalty:number;scoreNote:string|null;completedWithoutStart:boolean}}>(`/api/missions/${selectedMission.id}/complete`, "POST", { destinationName: destinationName.trim() || selectedMission.destinationName || "مقصد ثبت‌شده", result: workResult, report: workReport.trim(), expenseAmount: expenseEnabled ? parseExpenseAmount(expenseAmount) : 0, endLocation:latestGps });
  ...
  return <main className="admin-app" dir="rtl">
    ...
    <section className="admin-main">
      ...
      <div className="admin-content">
        ...
        {screen === "notifications" && <section className="admin-settings-layout"><NotificationCenter onOpenMissions={()=>setScreen("missions")} onCounts={setAdminNotificationCounts}/></section>}
