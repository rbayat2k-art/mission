...
type ApiReportDailyPoint = {date:string;activeMinutes:number;completedCount:number;successfulCount:number;travelMinutes:number;onSiteMinutes:number;missionDistanceKm:number;distanceKm:number;measuredMissionCount:number;firstStartAt:string|null;lastEndAt:string|null;hasActiveSession:boolean;gpsGapMinutes:number;internetGapMinutes:number};
...
function AdminPanel() {
  ...
  const [adminMissionSort, setAdminMissionSort] = useState<"newest"|"oldest"|"deadline"|"priority"|"employee"|"status">("newest");
  ...
  const loadAdminData = async (target = screen) => {
    ...
    if (target === "reports") {
      ...
      setReportRows(report.rows); setReportTotals(report.totals); setReportDailySeries(report.dailySeries); setReportComparison(report.comparison); setReportPolicy(report.policy); setReportDestinations(destinations.destinations);
    }
  };
