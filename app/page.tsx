...
import MissionBriefAttachments from "./components/MissionBriefAttachments";
...
import { createGpsAcquisition, gpsProgressMessage, type GpsProgress } from "../lib/gps-acquisition";
import { createWorkStartDeadline } from "../lib/work-start-deadline";
...
function MissionTaskChecklist({accountId,mission,location,onUpdate,onContinue,onMessage,onQueued}:{accountId:string;mission:UiMission;location:{latitude:number;longitude:number;accuracy:number;recordedAt:string}|null;onUpdate:(task:ApiMissionTask)=>void;onContinue:(result:string,report:string)=>void;onMessage:(message:string)=>void;onQueued:()=>void|Promise<void>}) {
  ...
  const [result,setResult]=useState(activeTask?.result??"");
  ...
  const selectTask=(task:ApiMissionTask)=>{setActiveId(task.id);setResult(task.result??"");setReport(task.report??"")};
  const save=async()=>{
    ...
    }catch(error){onMessage(error instanceof Error?error.message:"ثبت نتیجه کار ناموفق بود")}finally{savingRequest.current=false;setSaving(false)}
  };
  const finishList=()=>{
...
function EmployeeApp() {
  ...
  const [workGpsProgress, setWorkGpsProgress] = useState<GpsProgress | null>(null);
  ...
  const workStartOperation = useRef<{ cancel: () => void; checkDeadline: () => boolean } | null>(null);
  const workStartGeneration = useRef(0);
  const pendingWorkStartId = useRef<string | null>(null);
  useEffect(() => {
    const generation = workStartGeneration;
    const reset = window.setTimeout(() => { setWorkToggleBusy(false); setWorkSubmitting(false); setMissedStartSaving(false); setWorkGpsProgress(null); }, 0);
    const resume = () => { workStartOperation.current?.checkDeadline(); };
    ...
    return () => {
      ...
      generation.current++;
      workStartOperation.current?.cancel();
      workStartOperation.current = null;
      pendingWorkStartId.current = null;
      workStartRequest.current = false;
    };
  }, [employeeUserId, signedIn]);
  ...
  const [newMissionSaving, setNewMissionSaving] = useState(false);
  const newMissionSubmit = useRef(false);
  ...
  const syncQueued = useCallback(async () => {
    ...
    if (result.conflicts.length > 0) {
      setToast(result.conflicts[0].status === 409
        ? "یک تغییر آفلاین با اطلاعات جدید سرور تداخل دارد؛ اطلاعات حذف نشده و تا تصمیم شما دوباره ارسال نمی‌شود."
        : "سرور یک تغییر آفلاین را نپذیرفت؛ اطلاعات حذف نشده و برای بررسی شما محفوظ است.");
      return result;
    }
    if (result.sent > 0) {
  ...
  const addMission = async (e: FormEvent) => {
    e.preventDefault();
    if (!newTitle.trim() || newMissionSubmit.current) return;
    newMissionSubmit.current = true;
    setNewMissionSaving(true);
    try {
      await api("/api/missions", { method: "POST", headers: { "X-Tapra-User-Id": employeeUserId }, body: JSON.stringify({ title: newTitle, description:newDescription, referrerName: newReferrerName.trim() || null, priority:newPriority, deadlineDate:newDeadlineDate||null, deadlineTime:newDeadlineTime||null }) });
      setNewReferrerName("");
      setNewTitle("");
      setNewDescription("");setNewPriority("normal");setNewDeadlineDate("");setNewDeadlineTime("");
      setScreen("missions");
      try { await loadEmployeeData(); notify("مأموریت جدید در سرور ثبت شد"); }
      catch { notify("مأموریت ثبت شد، ولی تازه‌سازی فهرست ناموفق بود؛ دوباره مأموریت نسازید."); }
    } catch (error) { notify(error instanceof Error ? error.message : "ثبت مأموریت ناموفق بود"); }
    finally { newMissionSubmit.current=false;setNewMissionSaving(false); }
  };
  ...
  const prepareMissionWork = (mission: UiMission) => {
    ...
    setScreen("work");
    setWorkStep(missionWorkEntryStep(mission));
  };
  ...
  const registerDestination = async () => {
    ...
    try {
      ...
      setMissions(current=>current.map(mission=>mission.id===selectedMission.id ? updatedMission : mission));
      setWorkStep(1);
      if (result.queued) {
    ...
    finally { destinationRequest.current=false;setDestinationSaving(false); }
  };
  ...
  const startMission = async (mission: UiMission) => {
    ...
    try {
      ...
      const startedMission = { ...mission, backendStatus:"in_progress", status:"in_progress", startedAt: result?.mission.startedAt ?? mission.startedAt ?? new Date().toISOString(), completedAt:null,
        ...
        steps:mission.steps?.map(item=>item.stepNo===Number(mission.currentStepNo??1)&&mission.backendStatus!=="in_progress"?{...item,status:"in_progress",startedAt:item.startedAt??result?.mission.startedAt??new Date().toISOString()}:item),
    ...
    finally { missionStartRequest.current=false;setMissionStarting(false); }
  };
  ...
  const reportMissionWithoutStart = (mission: UiMission) => {
    ...
    if (!mission.startedAt && mission.backendStatus !== "in_progress" && !window.confirm("شروع این مأموریت ثبت نشده است. اگر از این مسیر آن را پایان دهید، طبق قانون فعلی ۳ امتیاز کسر می‌شود. با آگاهی از این موضوع ادامه می‌دهید؟")) return;
  ...
  const finishWork = async () => {
    ...
    finally { missionFinishRequest.current=false;setMissionFinishing(false); }
  };
  ...
  const startNextMissionStep = async () => {
    ...
    missionStartRequest.current=true;setMissionStarting(true);
    try {
      const result = await api<{mission:{startedAt:string}}>(`/api/missions/${selectedMission.id}/start`,{method:"POST",headers:{"X-Tapra-User-Id":employeeUserId},body:JSON.stringify({location:latestGps})});
  ...
  const captureFreshGps = async () => {
    const generation = workStartGeneration.current;
    const acquisition = createGpsAcquisition({
      ...
      onProgress: progress => {
        if (generation !== workStartGeneration.current) return;
        setWorkGpsProgress(progress);
        setGpsAccuracy(progress.accuracy);
        setGpsStatus(progress.phase === "READY" ? "ready" : progress.phase === "ACQUIRING" ? "requesting"
          : progress.phase === "WAITING_FRESH" || progress.phase === "STALE" ? "stale"
          : ["WAITING_ACCURACY", "LOW_ACCURACY", "PRECISE_REQUIRED"].includes(progress.phase) ? "approximate"
          : progress.phase === "PERMISSION_DENIED" ? "denied" : progress.phase === "TIMEOUT" ? "timeout" : "unavailable");
      },
      onDiagnostic: event => console.info("[tapra:gps-acquisition]", event),
    });
    workStartOperation.current = acquisition;
    try {
      const location = await acquisition.promise;
      if (generation !== workStartGeneration.current) throw new Error("دریافت موقعیت متوقف شد");
      setLatestGps(location);
      return location;
    } finally {
      if (workStartOperation.current === acquisition) workStartOperation.current = null;
    }
  };

  const adoptActiveWorkSession = async (generation: number) => {
    const request = createWorkStartDeadline(signal => api<ApiWorkState>("/api/work-sessions", { cache:"no-store", signal }));
    workStartOperation.current = request;
    const state = await request.promise;
    if (generation !== workStartGeneration.current) return false;
  ...
  const toggleWork = async () => {
    ...
    try {
      ...
      // Preserve the id across uncertain responses. Retrying cannot create a second session.
      const clientSessionId = pendingWorkStartId.current ?? createClientId();
      pendingWorkStartId.current = clientSessionId;
      const request = createWorkStartDeadline(signal => sendJsonOrQueue<{session:{id:string;startedAt:string;workType?:string;status?:string}}>(employeeUserId, "/api/work-sessions", "POST", { action: "start", clientSessionId, location }, { signal }));
      workStartOperation.current = request;
      const result = await request.promise;
      if (generation !== workStartGeneration.current) return;
      if (result.data?.session.status === "ended") {
        pendingWorkStartId.current = null;
        throw new Error("درخواست قبلی مربوط به فعالیت پایان‌یافته بود؛ برای شروع فعالیت جدید دوباره تلاش کنید.");
      }
      ...
      pendingWorkStartId.current = null;
      void getOutboxCount(employeeUserId).then(count => { if (generation === workStartGeneration.current) setPendingSync(count); }).catch(() => undefined);
      ...
      if (error instanceof ApiResponseError && error.status === 409 && error.code === "ACTIVE_WORK_SESSION_EXISTS") {
        try { if (await adoptActiveWorkSession(generation)) return; }
        catch { /* Preserve the original conflict message if authoritative state cannot be read. */ }
      }
      if (generation === workStartGeneration.current) notify(error instanceof Error ? error.message : "عملیات ناموفق بود");
    }
    finally {
      if (generation === workStartGeneration.current) {
        workStartOperation.current = null;
        workStartRequest.current = false; setWorkToggleBusy(false); setWorkSubmitting(false);
      }
    }
  };

  const logoutEmployee = async () => {
    // Cancel before awaiting logout: a late GPS fix must not send a new start meanwhile.
    workStartGeneration.current++;
    workStartOperation.current?.cancel();
    workStartOperation.current = null;
    workStartRequest.current = false;
    pendingWorkStartId.current = null;
    setWorkToggleBusy(false); setWorkSubmitting(false); setMissedStartSaving(false); setWorkGpsProgress(null);
    syncNativeTracking(false); clearNativeAuthenticatedUser();
    await detachPushDevice();
    await api("/api/auth/logout", {method:"POST"});
    setEmployeeUserId(""); setPendingSync(0); setSyncConflicts([]); setSyncQuarantined([]);
    setScreen("home"); setPassword(""); setSignedIn(false);
  };

  const submitMissedStart = async (event: FormEvent) => {
    event.preventDefault();
    if (workStartRequest.current) return;
    ...
    } catch (error) { if (generation === workStartGeneration.current) notify(error instanceof Error ? error.message : "ثبت خوداظهاری ناموفق بود"); }
    finally { if (generation === workStartGeneration.current) { workStartRequest.current = false; setMissedStartSaving(false); } }
  };
    ...
    <main className="employee-stage" dir="rtl">
      ...
      <section className="phone-shell" aria-label="نمونه اپلیکیشن کارمند">
        ...
        <div className="app-content">
          {screen === "home" && (
            <>
              ...
              <section className={`work-card ${working ? "active" : ""}`}>
                ...
                <button className={`work-toggle ${working ? "stop" : "start"}`} onClick={toggleWork} disabled={workToggleBusy || missedStartSaving}><span>{working ? "■" : workToggleBusy ? "⌖" : "▶"}</span>{working ? "پایان فعالیت" : workToggleBusy ? workSubmitting ? "در حال ثبت فعالیت..." : "در حال دریافت موقعیت دقیق..." : "شروع فعالیت"}</button>
                {!working && workGpsProgress && <div className="work-gps-feedback" role="status" aria-live="polite">
                  <p>{workSubmitting ? "موقعیت دریافت شد؛ در انتظار پاسخ ثبت فعالیت هستیم." : gpsProgressMessage(workGpsProgress)}</p>
                  {!workToggleBusy && !missedStartSaving && isNativeAndroidApp() && ["PERMISSION_DENIED", "PRECISE_REQUIRED", "LOCATION_DISABLED"].includes(workGpsProgress.phase) && <button type="button" onClick={() => {
                    try {
                      const bridge = androidBridge();
                      if (workGpsProgress.phase === "LOCATION_DISABLED" && bridge?.openDeviceLocationSettings) bridge.openDeviceLocationSettings();
                      else if (workGpsProgress.phase === "PRECISE_REQUIRED" && bridge?.requestPreciseLocationPermission) bridge.requestPreciseLocationPermission();
                      else bridge?.openLocationSettings();
                    } catch { notify("تنظیمات Location گوشی را باز کنید و پس از اصلاح، دوباره تلاش کنید."); }
                  }}>تنظیمات موقعیت</button>}
                </div>}
              </section>
          ...
          {screen === "missions" && (
            <>
              <p className="mission-safety-note">برای کار با برنامه، در جای امن توقف کنید.</p>
          ...
          {screen === "new" && (
            <form className="mobile-form" onSubmit={addMission}>
              ...
              <label>ارجاع‌دهنده کار <small>اختیاری</small><input disabled={newMissionSaving} value={newReferrerName} onChange={e => setNewReferrerName(e.target.value)} maxLength={255} placeholder="مثلاً: آقای احمدی، واحد مالی یا مشتری" /><small className="field-help">اگر این کار را شخصی مستقیم به شما ارجاع داده، نام او را ثبت کنید.</small></label>
              <label>عنوان مأموریت <b>*</b><input disabled={newMissionSaving} maxLength={255} value={newTitle} onChange={e => setNewTitle(e.target.value)} placeholder="مثلاً: پیگیری بیمه خودرو" required /></label>
              <label>توضیحات <small>اختیاری</small><textarea disabled={newMissionSaving} maxLength={4000} value={newDescription} onChange={event=>setNewDescription(event.target.value)} placeholder="جزئیات لازم برای انجام کار..." /></label>
              <div className="two-fields"><label>اولویت<select disabled={newMissionSaving} value={newPriority} onChange={event=>setNewPriority(event.target.value)}><option value="normal">عادی</option><option value="urgent">فوری</option><option value="low">کم</option></select></label><label>تاریخ مهلت <small>اختیاری</small><input disabled={newMissionSaving} value={newDeadlineDate} onChange={event=>setNewDeadlineDate(event.target.value)} placeholder="۱۴۰۵/۰۷/۰۱" /></label><label>ساعت مهلت <small>همراه تاریخ</small><input disabled={newMissionSaving} value={newDeadlineTime} onChange={event=>setNewDeadlineTime(event.target.value)} placeholder="۱۴:۳۰" /></label></div>
              <div className="info-box"><Icon>i</Icon><p>این مأموریت نیاز به تأیید اولیه ندارد؛ پس از انجام برای سرپرست ارسال می‌شود.</p></div>
              <button className="primary-wide" disabled={newMissionSaving} type="submit">{newMissionSaving?"در حال ثبت...":"ثبت و ادامه"}</button>
            </form>
          )}

          {screen === "work" && (
            <div className="work-flow">
              <div className="work-mission-context"><small>در حال انجام همین مأموریت</small><strong>{selectedMission.title}</strong></div>
              <button className="back-link" onClick={() => workStep===2||workStep===3 ? setWorkStep(workStep-1) : setScreen(workStep===4?"missions":"mission-detail")}>{workStep===2||workStep===3?"→ ویرایش مرحله قبل":workStep===4?"→ مأموریت‌ها":"→ بازگشت به همین مأموریت"}</button>
              ...
              <div className="stepper" aria-label="مراحل ثبت مأموریت">{["رسیدن","نتیجه","مدارک اختیاری","تأیید پایان"].map((label,s)=><span key={label} className={workStep>=s?"active":""} aria-current={workStep===s?"step":undefined}><i>{workStep>s?"✓":s+1}</i><b>{label}</b></span>)}</div>
              {workStep === 0 && <section className="flow-panel">
                <span className="flow-icon">⌖</span><h2>ثبت مقصد</h2><p>فقط وقتی به مقصد رسیدید و در جای امن توقف کردید، «ثبت مقصد و ادامه» را بزنید.</p>
          ...
          {screen === "mission-detail" && <div className="mission-detail-screen">
            ...
            <MissionBriefAttachments key={`${employeeUserId}:${selectedMission.id}`} accountId={employeeUserId} missionId={String(selectedMission.id)} />
            {!["awaiting_supervisor","awaiting_employee","escalated"].includes(selectedMission.followUpRequestStatus??"") && <>
            {["open","in_progress","follow_up"].includes(selectedMission.status) && selectedMission.backendStatus !== "follow_up_pending" && <section className="mission-detail-actions mission-focus-actions" aria-label="قدم بعدی مأموریت">
              <div className="mission-next-copy"><small>قدم بعدی شما</small><h3>{selectedMission.backendStatus!=="in_progress"?"شروع این مأموریت":missionWorkEntryStep(selectedMission)===1?"ثبت نتیجه کار":"رسیدن به مقصد"}</h3><p>{selectedMission.backendStatus!=="in_progress"?"شرح کار و عکس‌های بالا را ببینید، سپس شروع را بزنید.":missionWorkEntryStep(selectedMission)===1?(currentMissionStep(selectedMission)&&!currentMissionStep(selectedMission)?.requiresLocation?"این مرحله نیاز به ثبت مقصد ندارد؛ نتیجه کار را مشخص کنید.":"رسیدن شما ثبت شده؛ نتیجه کار را مشخص کنید."):"وقتی در مقصد توقف کردید، رسیدن خود را ثبت کنید."}</p></div>
              <ol className="mission-journey" aria-label="مراحل مأموریت">{["شروع","رسیدن","نتیجه"].map((label,index)=><li key={label} aria-current={index===(selectedMission.backendStatus!=="in_progress"?0:missionWorkEntryStep(selectedMission)===1?2:1)?"step":undefined}><span>{index+1}</span>{label}</li>)}</ol>
              ...
              <button className="primary-wide mission-main-action" disabled={!working || missionStarting || (selectedMission.backendStatus !== "in_progress" && activeMissionCount >= MAX_CONCURRENT_MISSIONS)} onClick={()=>startMission(selectedMission)}>{missionStarting?"در حال آماده‌سازی...":selectedMission.backendStatus==="in_progress"?(missionWorkEntryStep(selectedMission)===1?"ثبت نتیجه این مأموریت":"رسیدم؛ ثبت مقصد"):selectedMission.backendStatus==="follow_up"?"شروع پیگیری مجدد":"شروع این مأموریت"}</button>
              {!working&&<button className="mission-go-home" onClick={()=>setScreen("home")}>رفتن به شروع فعالیت روزانه</button>}
              {working&&selectedMission.backendStatus==="in_progress"&&missionWorkEntryStep(selectedMission)===1&&<small className="mission-shift-note">پایان این مأموریت، فعالیت روزانه شما را متوقف نمی‌کند.</small>}
              <details className="mission-other-actions" key={`other:${employeeUserId}:${selectedMission.id}`}><summary>گزینه‌های دیگر</summary>
                {selectedMission.backendStatus!=="in_progress"&&<><p>شروع را فراموش کرده‌اید؟ ثبت از مسیر زیر ممکن است طبق قانون فعلی کسر امتیاز داشته باشد.</p><button className="mission-report-direct" disabled={!working||missionStarting} onClick={()=>reportMissionWithoutStart(selectedMission)}>ثبت نتیجه بدون شروع مأموریت</button><small className="direct-score-note">در صورت پایان مأموریت از این مسیر، ۳ امتیاز کسر و دلیل آن در گزارش شما ثبت می‌شود.</small></>}
            ...
            <details className="mission-history" key={`history:${selectedMission.id}`}><summary>سوابق و جزئیات بیشتر</summary>
            ...
            {selectedMission.id && selectedMission.followUpRequestStatus && selectedMission.result && selectedMission.result !== "انجام شد" && <EmployeeFollowUpPanel key={`${employeeUserId}:${selectedMission.id}`} accountId={employeeUserId} missionId={String(selectedMission.id)} onMessage={notify}/>}
          ...
          {screen === "notifications" && <NotificationCenter key={employeeUserId} accountId={employeeUserId} onOpenMissions={()=>{loadEmployeeData().catch(()=>undefined);setScreen("missions")}} onOpenFollowUps={()=>{loadEmployeeData().catch(()=>undefined);setMissionTab("follow_up");setScreen("missions")}} onCounts={setNotificationCounts}/>}
  ...
  return <main className="admin-app" dir="rtl">
    ...
    <section className="admin-main">
      ...
      <div className="admin-content">
        ...
        {screen === "actions" && <FollowUpActionCenter key={adminUserId} accountId={adminUserId} onMessage={notify} onCountChange={updateFollowUpCount}/>}
